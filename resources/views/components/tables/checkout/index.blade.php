                        ...
                        <thead class="text-xs font-semibold uppercase text-gray-400 bg-gray-50">
                            <tr>
                                <th class="p-2 whitespace-nowrap">
                                    <div class="font-semibold text-right">رقم الطلب</div>
                                </th>
                                <th class="p-2 whitespace-nowrap">
                                    <div class="font-semibold text-right">رقم البطاقة</div>
                                </th>
                                <th class="p-2 whitespace-nowrap">
                                    <div class="font-semibold text-right">إجمالي السعر</div>
                                </th>
                                <th class="p-2 whitespace-nowrap">
                                    <div class="font-semibold text-right">طريقة الدفع</div>
                                </th>
                                <th class="p-2 whitespace-nowrap">
                            ...
                            @forelse ( $results as $item )
                                <tr wire.loading.class.delay="opacity-50" wire:key="{{ $loop->index }}">
                                    <td class="p-2 whitespace-nowrap">
                                        <div class="font-medium text-gray-800">{{ $item['order_number'] }}</div>
                                    </td>
                                    <td class="p-2 whitespace-nowrap">
                                        @if ($item['payment_number'] == '0000')
                                            <div class="text-xs text-gray-400 italic">                                 لا يوجد رقم بطاقة</div>
                                        @else
                                            <div class="font-medium text-gray-800">{{ $item['payment_number'] }}</div>
                                        @endif
                                    </td>
                                    <td class="p-2 whitespace-nowrap">
                                        <div class="font-medium text-gray-800">{{ $item['total_price'] }}</div>
                                    </td>
                                    <td class="p-2 whitespace-nowrap">
                                        <div class="font-medium text-gray-800">{{ $item['paymentMethod']['name'] }}</div>
                                    </td>
                                    <td class="p-2 whitespace-nowrap">
                                    ...
                                    {{-- <td class="p-2 whitespace-nowrap">
                                        <form
                                            ...
                                            action="{{ route('checkouts.destroy', $item['id']) }}"
                                            class="flex"
                                            >
                                            @csrf
                                            @method('DELETE')

                                            <a href="{{ route('checkouts.edit', $item['id'])}}" class="btn btn-primary">
